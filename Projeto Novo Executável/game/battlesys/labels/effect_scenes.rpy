...
label hp_effect_scene:
    if effect.target == 'player':
        if effect.operator == '+':
            ...
            narrator "[playerName]'s HP was restored."
        elif effect.operator == '-':
            $ battleState.player_hp = max(0, battleState.player_hp - effect.value)
            narrator "[playerName]'s is damaged by recoil." with vpunch
        elif effect.operator == '*':
            $ battleState.player_hp = min(battleState.original_player.hp, max(0, battleState.player_hp * effect.value))
            if (effect.value > 1): 
                narrator "[playerName]'s HP rose!" 
            else: 
                narrator "[playerName]'s HP fell!"
    elif effect.target == 'enemy':
        if effect.operator == '+':
...
label atk_effect_scene:
    if effect.target == 'player':
        if effect.operator == '+':
            ...
            narrator "[playerName]'s Attack rose!"        
        elif effect.operator == '-':
            $ battleState.player_atk = max(1, battleState.player_atk - effect.value)
            narrator "[playerName]'s Attack fell!"
        elif effect.operator == '*':
...
label res_effect_scene:
    if effect.target == 'player':
        if effect.operator == '+':
            ...
            narrator "[playerName]'s Defense rose!"        
        elif effect.operator == '-':
            $ battleState.player_res = max(1, battleState.player_res - effect.value)
            narrator "[playerName]'s Defense fell!"
        elif effect.operator == '*':
        ...
        if effect.operator == '+':
            python:
                if not effect.target_type in keys:
                    ...
                    battleState.type_boost_dictionare[effect.target_type] += 1
            narrator "[playerName]'s [type_name] efficiency increased!"        
        elif effect.operator == '-':
            python:
                if not effect.target_type in keys:
                    battleState.type_boost_dictionare[effect.target_type] = -1
                    ...
                    battleState.type_boost_dictionare[effect.target_type] -= 1
            narrator "[playerName]'s [type_name] efficiency decreased!"  
    #enemy
    elif effect.target == 'enemy':
...
label status_condition_effect_scene:
    ...
    if effect.target == 'player':
        if effect.operator == '+':
            ...
            narrator "[playerName]'s was affected by [condition.name]!"        
        elif effect.operator == '-':
            python:
                keys = battleState.player_status_condition_dictionare.keys()
                if not condition in keys:
                    renpy.say('', '[playerName] is not affected by [condition.name]!')
                else:
                    battleState.player_status_condition_dictionare.pop(condition)
                    renpy.say('', '[playerName] is no longer affected by [condition.name]!')  
    #enemy
    elif effect.target == 'enemy':
        if effect.operator == '+':
